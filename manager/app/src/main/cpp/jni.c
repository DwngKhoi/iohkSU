...
NativeBridge(getAppProfile, jobject, jstring pkg, jint uid) {
	...
	jclass cls = GetEnvironment()->FindClass(env, "com/iohksu/yakayn/Natives$Profile");
...
NativeBridgeNP(getKernelPatchImplementation, jobject) {
	...
	jclass cls = GetEnvironment()->FindClass(env,
                                             "com/iohksu/yakayn/Natives$KernelPatchImplementation");
	if (cls == nullptr) {
	...
	jmethodID valuesMethod = GetEnvironment()->GetStaticMethodID(env, cls, "values",
                                                                 "()[Lcom/iohksu/yakayn/Natives$KernelPatchImplementation;");
...
NativeBridgeNP(getDynamicManager, jobject) {
	...
	jobject obj = CREATE_JAVA_OBJECT("com/iohksu/yakayn/Natives$DynamicManagerConfig");
	jclass cls = GetEnvironment()->FindClass(env, "com/iohksu/yakayn/Natives$DynamicManagerConfig");
...
NativeBridgeNP(getManagersList, jobject) {
    ...
    jobject obj = CREATE_JAVA_OBJECT("com/iohksu/yakayn/Natives$ManagersList");
    jclass managerListCls = GetEnvironment()->FindClass(env,
                                                        "com/iohksu/yakayn/Natives$ManagersList");
    ...
    if (cmd && count > 0) {
        for (int i = 0; i < count; i++) {
            jobject managerInfo = CREATE_JAVA_OBJECT_WITH_PARAMS(
                    "com/iohksu/yakayn/Natives$ManagerInfo",
                    "(II)V",
...
Java_com_iohksu_yakayn_magica_AppZygotePreload_forkDontCareAndExecKsud(JNIEnv *env,
                                                                           jclass clazz,
