package com.dwngkhoi.iohksu.data.text
...
class HanziToPinyin private constructor(val hasChinaCollator: Boolean) : TextTransliterator {
    ...
    override fun transliterate(value: String): String = toPinyinString(value).orEmpty()
    ...
    companion object {
        ...
        fun create(): HanziToPinyin {
            val hasChineseCollator = Collator.getAvailableLocales().any { locale ->
                locale == Locale.CHINA || locale.language.contains("zh")
            } || Locale.CHINA == Locale.getDefault()
            if (hasChineseCollator && DEBUG) {
                Log.d(TAG, "Self validation. Result: ${doSelfValidation()}")
            }
            if (!hasChineseCollator) {
                Log.w(TAG, "There is no Chinese collator, HanziToPinyin is disabled")
            }
            return HanziToPinyin(hasChineseCollator)
        }
